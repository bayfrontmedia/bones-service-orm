 ...
 * @inheritDoc
 */
protected function getDeletedAtField(): array
{
    return 'deleted_at';
}
```
...
If successful, the `orm.resource.restore` [event](../events.md) is executed.
...
- [OrmResource](../ormresource.md)|(null): Returns `null` if resource does not exist
...
If successful, the `orm.resource.delete` [event](../events.md) is executed.
...
The `orm.resource.delete` [event](../events.md) is executed for each deleted resource.
...
so the `orm.resource.delete` [event](../events.md) is not executed.
