...
class QueryParser implements QueryParserInterface
{
    ...
    private function validateQuery(): void
    {
        ...
        $fields = Arr::get($this->query, 'fields', []);

        if (!is_array($fields)) {
            throw new InvalidRequestException('Unable to parse request: Invalid fields format');
        }
        ...
        if (!is_array(Arr::get($this->query, 'filter', []))) {
            throw new InvalidRequestException('Unable to parse request: Invalid filter format');
        }
        ...
        if (!is_array(Arr::get($this->query, 'sort', []))) {
            throw new InvalidRequestException('Unable to parse request: Invalid sort format');
        }
        ...
        if (!is_array(Arr::get($this->query, 'group', []))) {
            throw new InvalidRequestException('Unable to parse request: Invalid group format');
        }
        ...
        if (Arr::has($this->query, 'aggregate')) {

            $aggregate = Arr::get($this->query, 'aggregate');

            if (!is_array($aggregate)) {
                throw new InvalidRequestException('Unable to parse request: Invalid aggregate format');
            }
    ...
    {
        return Arr::get($this->query, 'fields', []);
    }
    ...
    {
        return Arr::get($this->query, 'filter', []);
    }
    ...
    {
        return Arr::get($this->query, 'sort', []);
    }
    ...
    {
        return Arr::get($this->query, 'group', []);
    }
    ...
    {
        return Arr::get($this->query, 'aggregate', []);
    }
